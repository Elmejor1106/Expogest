...
public class EventoController {
    ...
    @GetMapping
    public String listarEventos(@RequestParam(required = false) String buscar, Model model, HttpSession session) {
        // Público: todos pueden ver la lista de eventos (sin restricción de rol)
        ...
        // Pasar rol al modelo para ajustar botones en la vista
    ...
    public String verStandsDelEvento(@PathVariable String id, Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        ...
        // Para organizadores: gestión completa de stands
        if ("ORGANIZADOR".equals(rol)) {
            model.addAttribute("evento", evento);
            model.addAttribute("stands", eventoService.obtenerStandsDelEvento(id));
            model.addAttribute("standsDisponibles", eventoService.obtenerStandsDisponiblesParaAsociar(id));
            return "evento/stands";
        }
        
        // Para visitantes/evaluadores: ver stands con material comercial
        List<Stand> stands = eventoService.obtenerStandsDelEvento(id);
        model.addAttribute("evento", evento);
        model.addAttribute("stands", stands);
        
        // Agregar material comercial de cada stand
        ...
        model.addAttribute("materialesPorStand", materialesPorStand);
